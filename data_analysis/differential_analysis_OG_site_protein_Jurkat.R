...
#generate data frame
OG_site_protein_OGlcNAc_effect_Jurkat <- OG_glycopeptide_raw_sl_tmm_Jurkat |> 
...
data_matrix <- data.matrix(OG_site_protein_OGlcNAc_effect_Jurkat |> select(starts_with("log2")))
rownames(data_matrix) <- OG_site_protein_OGlcNAc_effect_Jurkat$Index
...
top_tb$Index <- rownames_Jurkat

OG_site_protein_OGlcNAc_effect_Top_tb_Jurkat <- top_tb |> 
  left_join(OG_site_protein_OGlcNAc_effect_Jurkat |> select(Index, UniprotID, combined_site), by = "Index") |> 
  mutate(
    site_position = str_extract(combined_site, "\\d+"),
    site_position = as.numeric(site_position)
  )

write_xlsx(OG_site_protein_OGlcNAc_effect_Top_tb_Jurkat, path = paste0(file_path, "OG_site_protein_OGlcNAc_effect_Top_tb_Jurkat.xlsx"))
