...
#generate data frame
OG_glycopeptide_Top_tb_singlesite_secondary_structure_HepG2 <- OG_glycopeptide_Top_tb_HepG2_singlesite |> 
...
OG_glycopeptide_Top_tb_singlesite_secondary_structure_HEK293T <- OG_glycopeptide_Top_tb_HEK293T_singlesite |> 
  mutate(Protein_Site = paste(UniprotID, combined_site, sep = "_"),
         Cell = "HEK293T") |> 
  select(Index, Protein_Site, UniprotID, combined_site, logFC, adj.P.Val, Cell) |> 
  left_join(OG_glycoprotein_localized_glycoprotein_secondary_structure_combined_tb, by = join_by(Protein_Site == Protein_Site))

OG_glycopeptide_Top_tb_singlesite_secondary_structure_Jurkat <- OG_glycopeptide_Top_tb_Jurkat_singlesite |> 
  mutate(Protein_Site = paste(UniprotID, combined_site, sep = "_"),
         Cell = "Jurkat") |> 
  select(Index, Protein_Site, UniprotID, combined_site, logFC, adj.P.Val, Cell) |> 
  left_join(OG_glycoprotein_localized_glycoprotein_secondary_structure_combined_tb, by = join_by(Protein_Site == Protein_Site))
...
ggsave(filename = paste0(file_path, "violin_point_OG_site_secondary.png"), plot = violin_point_OG_site_secondary , height = 4, width = 6, units = "in", dpi = 600)
...
violin_point_OG_glycosite_BE <- OG_glycopeptide_localized_singlesite_secondary_structure_combined |> 
  ...
  theme(
    ...
    axis.text.x = element_text(size = 10, color = "black", angle = 30, hjust = 1, lineheight = 0.1),
...
ggsave(
  filename = paste0(file_path, "violin_point_OG_glycosite_BE.eps"), 
  plot = violin_point_OG_glycosite_BE,
  device = 'eps',
...
ggsave(filename = paste0(file_path, "violin_point_OG_glycosite_disorder.eps"), 
       plot = violin_point_OG_glycosite_disorder, 
       height = 2.8, width = 2.5, 
       units = "in", 
       dpi = 1200)

#O-GlcNAcylation site example
N_glycoprotein_dataset <- read_xlsx(
  "E:\\Weekly Reading\\20241125\\41467_2018_4081_MOESM5_ESM.xlsx",
  sheet = 'Sheet1',
  col_names = 'UniprotID'
)

OG_glycosite_disorder_N_glyco_tb <- read_xlsx(
  "E:\\Glycosylation_Crosstalk\\Data_Analysis_Sequest\\data_source\\OG_glycosite_disorder_tb.xlsx"
) |> 
  filter(Disorder == "Ordered", logFC > 0.5, adj.P.Val < 0.05) |> 
  select(Index, Protein_site, UniprotID = UniprotID.x, combined_site, logFC, adj.P.Val, Cell, disorder) |> 
  filter(UniprotID %in% N_glycoprotein_dataset$UniprotID)

write_xlsx(OG_glycosite_disorder_N_glyco_tb, path = paste0(file_path, "OG_glycosite_disorder_N_glyco_tb.xlsx"))

OG_glycosite_secondary_N_glyco_tb <- read_xlsx(
...
write_xlsx(OG_glycosite_secondary_N_glyco_tb, path = paste0(file_path, "OG_glycosite_secondary_N_glyco_tb.xlsx"))
