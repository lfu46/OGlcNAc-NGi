...
#generate data frame
OG_glycopeptide_singlesite_domain_HepG2 <- OG_glycopeptide_Top_tb_HepG2_singlesite |> 
...
OG_glycopeptide_singlesite_domain_HEK293T <- OG_glycopeptide_Top_tb_HEK293T_singlesite |> 
  mutate(site_position = str_extract(combined_site, "\\d+")) |> 
  left_join(OG_glycopeptide_superfamily_total_adj, by = "UniprotID", relationship = "many-to-many") |> 
  select(Index, UniprotID, Region, Start_adj, End_adj, combined_site, site_position, logFC, adj.P.Val) |> 
  mutate(Domain = ifelse(is.na(Region), "Not Domain", ifelse(site_position > Start_adj & site_position < End_adj, "Domain", "Not Domain"))) |> 
...
OG_glycopeptide_singlesite_domain_Jurkat <- OG_glycopeptide_Top_tb_Jurkat_singlesite |> 
  mutate(site_position = str_extract(combined_site, "\\d+")) |> 
  left_join(OG_glycopeptide_superfamily_total_adj, by = "UniprotID", relationship = "many-to-many") |> 
  select(Index, UniprotID, Region, Start_adj, End_adj, combined_site, site_position, logFC, adj.P.Val) |> 
  mutate(Domain = ifelse(is.na(Region), "Not Domain", ifelse(site_position > Start_adj & site_position < End_adj, "Domain", "Not Domain"))) |> 
...
ggsave(filename = paste0(file_path, "violin_point_OG_glycopeptide_singlesite_domain.png"), plot = violin_point_OG_glycopeptide_singlesite_domain, height = 4, width = 6, units = "in", dpi = 600)
